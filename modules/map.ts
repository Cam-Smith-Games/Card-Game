...
export class DungeonMap {
    ...
    /** this gets set to pass the selected node back to game state upon clicking on a node */
    callback:(node:DungeonNode)=>void;

    constructor(callback:(node:DungeonNode)=>void, length:number = 5) {
    ...
    connect(child:DungeonNode, parent:DungeonNode) {
    ...
    getClosest(from:DungeonNode, nodes:DungeonNode[]) {
...
class DungeonNode {
    ...
    constructor(map:DungeonMap, type:DungeonTypes, _click:()=>void) {
        this.map = map;
        this.type = type;
...
export interface MapCallbacks {
    MONSTER: (battle:Battle) => void;
}


function getDist(from:HTMLElement, to:HTMLElement, from_anchor:string = "center center", to_anchor:string = "center center") {
...
function getLine(
    from:HTMLElement,
    to:HTMLElement, 
    from_anchor:string = "center center", 
    to_anchor:string = "center center"
) 
{
    const pos1 = getPos(from, from_anchor);
    const pos2 = getPos(to, to_anchor); 
...
function getPos( el:HTMLElement, anchor:string ) {
    const anchors = anchor.split(" ");    
    return {
        x: getAnchorPos(el.offsetLeft, el.offsetWidth, anchors[0] ?? "center"),
        y: getAnchorPos(el.offsetTop, el.offsetHeight, anchors[1] ?? "center")
    };
}

function getAnchorPos(start:number, length:number, anchor:string) {
    return start + (
        anchor == ("left" || "top") ? 0 :
